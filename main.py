...
# =============================================================================
# SIMULATION 800 ANS (1800–2600)
# =============================================================================
t_start, t_end = 1800, 2600
N = 8000  # dt = 0.1 an

print("Simulation Euler 800 ans...")
t_e, X_e = euler(x0, t_start, t_end, N)
print("Simulation RK4 800 ans...")
...
def get_consistency_error(method_func, h_list):
    """Calcule l'erreur de consistance maximale pour différentes tailles de pas."""
    errors = []
    for h in h_list:
        # Test over the new full range (1800-2600)
        n = int((2600 - 1800) / h) 
        t, x = method_func(x0, 1800, 2600, n)
        # L'erreur de consistance est estimée par la différence locale normalisée
        # Ici on simplifie par l'écart entre un pas h et h/2 (Richardson)
        _, x_fine = method_func(x0, 1800, 2600, n*2)
        errors.append(np.max(np.abs(x[-1] - x_fine[-1])))
    return errors
...
err_euler = get_consistency_error(euler, h_values)
err_rk4 = get_consistency_error(rk4, h_values)
...
def stability_test(method_func, h):
    ...
    # Simulation normale
    _, y = method_func(x0, t_range[0], t_range[1], n)
    # Simulation perturbée
    noise = np.random.normal(0, 0.01, y.shape)
    y_perturbed = y + noise
    
    numerator = np.max(np.linalg.norm(y_perturbed - y, axis=1))
    denominator = np.linalg.norm(noise[0]) + np.sum(np.linalg.norm(noise, axis=1))
    return numerator / (denominator + 1e-10)

# Using h_values for stability test
h_stab_values = np.logspace(-2, -0.5, 10) # Example h values, adjust as needed

s_euler = [stability_test(euler, h) for h in h_stab_values]
s_rk4 = [stability_test(rk4, h) for h in h_stab_values]

plt.figure(figsize=(10, 4))
plt.plot(h_stab_values, s_euler, 'o-', label='Euler')
plt.plot(h_stab_values, s_rk4, 's-', label='RK4')
plt.xscale('log')
plt.yscale('log')
plt.title('Comparaison de la Constante de Stabilité S vs Taille du Pas (1800-2600)')
